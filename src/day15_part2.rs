...
fn get_answer(input: &str) -> Option<isize> {
    ...
    let mut elves_attack = 3;
    while !increase_attack(input,elves_attack){
        println!("{} is too low", elves_attack);
        elves_attack += 1;
        if elves_attack > 100 {
            break;
        }     
...
fn increase_attack(input: &str,elves_attack:isize) -> bool {
    // get grid
    let mut grid = Grid2D::new(input);

    // get units
    let mut units = get_units(&grid,elves_attack);
    ...
    loop {
        ...
        for su in &sorted_units {
            ...
            if v_opp.is_empty() && !v_free.is_empty() {
                // move
                if let Some(new_pos) = move_one_step(&grid, units[current]) {
            ...
            if !v_opp.is_empty() {
                ...
                if let Some(new_pos) = attack(&grid, &units, units[current]) {
                    ...
                    println!("opp has {} - {} hit points",units[opp_id].hit,units[current].attack);
                    units[opp_id].hit -= units[current].attack;
                    if units[opp_id].hit <= 0 {
                        println!("opp {:?} is dead",units[opp_id]);
                        if opponent == 'E' {
                            return false;
                        }
...
fn move_one_step(grid: &Grid2D, u: Unit) -> Option<(usize, usize)> {
    let opponent = match u.side {
        'G' => 'E',
        'E' => 'G',
        _ => panic!(),
    ...
    for (i, d) in dir.iter().enumerate() {
        if let Some(start_pos) = grid.is_next_valid(u.pos, *d) {
            if grid.get_at(start_pos) == '.' {
                if let Some(c) = bfs(grid, start_pos, opponent) {
                    cost[i] = c;
                }
            }
        }
...
fn bfs(grid: &Grid2D, start_pos: (usize, usize), opponent: char) -> Option<usize> {
...
fn get_units(grid: &Grid2D,elves_attack:isize) -> Vec<Unit> {
    let mut units: Vec<Unit> = Vec::new();
    add_unit(grid, &mut units, 'G',3);
    add_unit(grid, &mut units, 'E',elves_attack);
    units
}

fn add_unit(grid: &Grid2D, unit: &mut Vec<Unit>, side: char,attack:isize) {
